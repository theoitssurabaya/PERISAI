  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 h-14 sm:h-16 bg-white border-b border-gray-200 flex items-center justify-between px-3 sm:px-4">
      ...
      <div className="flex items-center gap-2 sm:gap-3">
        {isLoggedIn ? (
          <>
            <span className="hidden sm:block text-sm text-[#64748B]">Hi, {user?.name}</span>
            <button
              onClick={handleLogout}
              className="text-xs sm:text-sm text-[#64748B] hover:text-[#0F172A] font-medium px-2 sm:px-4 py-2 hover:underline"
            >
              Logout
            </button>
            <IoPersonCircleOutline size={26} className="text-[#0F172A] cursor-pointer" />
            <IoSettingsOutline size={22} className="hidden sm:block text-[#0F172A] cursor-pointer" />
          </>
        ) : (
          <>
            <button
              onClick={() => navigate('/register')}
              className="text-[#3B82F6] font-medium text-xs sm:text-sm px-2 sm:px-4 py-2 hover:underline"
            >
              Sign Up
            </button>
            <button
